...
| 74 | **The shipped release is broken for every scaffolded repo.** `release.yml` carries literal `<domain>` placeholders that nothing substitutes: `cd custom_components/<domain>`, the zip name, and the upload path. Bash reads `<` as a redirect, so the step dies with `syntax error near unexpected token 'newline'` and exits 2 — no asset is attached, and HACS installs fail with `Could not download`. Observed on `ha-ci-testing` publishing `v0.2.1rc1`: `Release Drafter` succeeded and `Create Release ZIP` failed. The manifest step at `:38` already solves this correctly with `glob("*/manifest.json")`; the zip step should derive the domain the same way rather than expecting a hand-edit | `templates/.github/workflows/release.yml:83-84,92` |
| 75 | Nothing detects an unsubstituted placeholder in a shipped template. `check_zip_release_patches_manifest` passes `release.yml` because it only asks whether the text contains `manifest.json`, which it does while the workflow is fatally broken. A check for `<...>` tokens in `templates/` would have caught row 74 before it ever reached a repo | `scripts/skill_audit.py` `check_zip_release_patches_manifest` |

| 76 | `check_required_contexts_have_producers` and `check_live_required_contexts` compare the ruleset's required contexts against jobs in the **local checkout**. Under `pull_request_target` the producing workflow comes from the **base branch**, so a branch that deletes a job `main` still defines is reported as an orphaned context when it is nothing of the kind. That misreading cost a working gate: `Version validation` was called an orphan and removed from this repo's ruleset, while `origin/main` still defined the `version-gate` job and it was correctly failing PR #65 for shipping `type!:` commits under a non-breaking title. The check should resolve producers from the base branch's workflows, and say which ref it judged | `scripts/skill_audit.py` `check_required_contexts_have_producers`, `check_live_required_contexts` |
| 77 | ~~Sequencing: the `Version validation` context may only be dropped after its replacement is on `main`.~~ **Withdrawn — overstated.** Dropping it was the plan (see `workflow-map.md` item 6: the job is deleted by design and the context goes with it). The only residue is that `main`'s `version-gate` keeps reporting, non-blocking, until the branch merges — and the sole PR affected is the one carrying the replacement. Worth noting only as context for row 76: on `main`'s stack `CC label validation` cannot fail (its steps emit `::warning::` and set an output, never exit 1), which is why the branch rewrites it to exit 1 | `.github/workflows/pr-checks.yml` on `origin/main:93-116` |

| 78 | **The governed-edit gate is machine-local while the instruction to use it is repo-global.** The repo carries the reminder hooks, `governance_gate.py`, `governance_server.py` and their tests; every piece of *enforcement* lives outside it — 31 `permissions.deny` entries and 7 `sandbox.filesystem.denyWrite` paths in `~/.claude/settings.json`, the MCP registration in `~/.claude.json`, and the SDK in a gitignored `.venv`. On a fresh clone the repo hook therefore tells a session to route writes through a gate that is not registered, has no interpreter to run it, and guards nothing — a reminder pointing at absent tools is worse than silence. Test whether `permissions.deny` and `sandbox.filesystem.denyWrite` are honoured from project settings; if they are, the denies belong in `.claude/settings.json` beside the hooks, leaving only the MCP registration and venv as per-machine setup (and those should fail loudly rather than silently) | `.claude/settings.json` vs `~/.claude/settings.json`, `~/.claude.json` |
