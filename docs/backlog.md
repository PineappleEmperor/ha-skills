...
| 80 | **A gate restart is invisible to the caller.** Every key is an HMAC over a salt drawn at process start, so when the harness restarts the server every outstanding receipt and edit key dies at once — and the refusal reads exactly like a stale key or an hour rollover. Observed 2026-09-03 ~19:00: keys issued minutes earlier, docs and file unchanged, well inside the grace window, all refused; re-fetching restored service. Nothing in the protocol lets the caller tell "you read a stale copy" from "the gate was reborn". Emit a short per-process gate id on every key line and in every refusal, so a mismatch names the cause | `scripts/governance_gate.py` `_SALT`, every refusal message |
| 81 | **A test string tripped the secrets scanner and locked its own file.** `tests/test_governance_gate.py` carries `"import pytest\n\n\n@pytest.fixture\ndef thing()..."` as one literal; the PII scanner reads `n@pytest.fixture` as an email address and now refuses every read of the file through every tool, including the gate, so the two-character fix (split the literal at the `@`) cannot be made without a manual allow tag. Any governed file can be locked the same way by content that merely looks like a secret; splitting such literals at authoring time is the only defence that needs no human | `tests/test_governance_gate.py` *test_a_fixture_named_as_a_parameter_is_part_of_the_closure* |
