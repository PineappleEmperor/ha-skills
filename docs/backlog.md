...
| 77 | On `main`'s stack `CC label validation` is required but **cannot fail** — its steps only emit `::warning::` and set an output, never exit 1 — so `Version validation` is the sole enforcing gate there. Removing that context leaves the base branch with a required check incapable of going red and nothing enforcing the label/breaking rule until the branch's rewritten `title-check` merges. Sequencing matters: the context may only be dropped *after* the job that replaces it is on `main` | `.github/workflows/pr-checks.yml` on `origin/main:93-116` |
