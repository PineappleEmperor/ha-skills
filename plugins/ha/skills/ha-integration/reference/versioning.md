...
>       replace: ''
>   ```
>   Leaves Dependabot's clean opener (`Bumps [pkg] from a to b.`) as the body — a fine one-liner. Regex over bot output is inherently brittle: revisit if Dependabot changes its format. (The workflow that builds the grouped `$BODY` is `templates/.github/workflows/pr-commit-summary.yml`; rationale in `reference/github-actions.md`.)
>
...
1. A **human** writes the PR **title**. `lint_pr.yml` gates its format; nothing sets it automatically. `pr-commit-summary.yml` only maintains the commit-list block in the body and does **no** labelling.
...
```
The `!`-breaking branch must come first (else `feat!` matches the `feat` arm). This is still **one source of truth** — the title — and removal-only, so it can't fight the autolabeler the way a second *adding* step does. Needs `pull-requests: write`. **Note this only fixes the *labels* (one PR → one category).** Within a single squash PR whose body lists mixed-type commits, the commits stay together under that PR's one category — sort *those* by grouping the PR body itself by commit type in `pr-commit-summary.yml` (bold emoji sub-heads), since release-drafter inlines `$BODY` verbatim under the category and does no intra-body sorting.

⚠️ **Type-vocab gap:** the autolabeler title rules map only `feat|fix|chore|docs` (+ `!` → `xfeat`). A title typed `ci:`, `refactor:`, `build:`, `perf:`, `style:`, `revert:` matches **nothing** → no label → no release-drafter category. **This now bites harder, because a human types the title.** `lint_pr` accepts `ci:`/`refactor:`/`perf:`/`build:`/`style:`/`revert:`, so a PR can pass the title lint and still get no label, no release category, and no bump for the version gate to check. Use a mapped type on the **title** (e.g. `chore:` not `ci:` for a workflow-only PR) whatever the individual commits say. Don't hand-patch the label — the autolabeler rewrites it on the next `synchronize`.
...
> ⚠️ **Orphaned-branch trap.** A PR merges to `main` as soon as it's approved/auto-merged. **Any commit you push to `feat/rcN` after that merge is stranded** — it's not on `main` and not in the release, even though `git status` on the branch looks fine. It also leaves the branch's manifest equal to `main`'s, so `check-manifest-version` fails. **Guard every time, not just when you remember:**
...
One piece of that old advice is worth keeping on its own merits: give `check-manifest-version` a `push:` trigger (`branches-ignore: [main]`) as well, so the version gate runs on the branch push and not only in PR context. Default the base ref to `main` when there's no PR (`${{ github.event.pull_request.base.ref || 'main' }}`) and guard PR-only steps with `if: github.event_name == 'pull_request'` (label lookups, PR comments).
