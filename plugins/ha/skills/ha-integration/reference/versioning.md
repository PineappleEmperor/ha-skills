...
**Match release-drafter when writing the PR body.** If `change-template` includes `$BODY`, the PR description is inlined **under** its category heading (e.g. `### 🚀 Features`). So the body must nest cleanly: use plain labels ending in a colon (`Engine:`), not `#`/`##`, which render bigger than the category and clash. Avoid boldface and emoji in the body as well; both read as machine-written, and the notes are meant to read as a person's. Label the PR so it lands in the intended category (e.g. a `major`/`xfeature` label → 🚨 Breaking Change). Note release-drafter draws the PR body via the GraphQL path; `gh pr edit` can fail on the Projects-classic deprecation — set title/body via `gh api -X PATCH repos/{o}/{r}/pulls/{n} -f title=… -F body=@file` instead.
...
>
> - **Group the PR body by commit type** in the `commit-summary` job of `pr-checks.yml`: classify each subject in the PR (`breaking`/`feat`/`fix`/`maint`/`other`), emit plain labels (`  Features:`, `  Fixes:`, `  Maintenance:`…) with the descriptions nested under each, into a marked block spliced into the PR body. **No boldface and no emoji**: both are reliable tells of machine-written text, and this block is inlined verbatim into release notes a human is expected to have authored. Every line keeps a two-space base indent so the block nests under the `- $TITLE …` bullet; strip that from the first line and the opening label sits flush left while the rest stay indented. release-drafter inlines `$BODY` verbatim under the PR's one category and does **no** intra-body sorting, so the grouping must happen at body-generation time.
