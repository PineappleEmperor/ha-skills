...
Use it when the task touches any of: a `custom_components/<domain>/` package, a `manifest.json` with a `domain`, a config/options/reauth/reconfigure flow, a `DataUpdateCoordinator` or entity platform (`sensor.py`, `notify.py`, …), `services.yaml`/`quality_scale.yaml`, the integration's GitHub CI (the `pr-commit-summary`/release-drafter/hassfest/HACS stack), or a Home Assistant log to triage. Symptoms that should pull you here: "add a sensor/platform", "config flow won't validate", "hassfest/HACS check failing", "what `state_class` for this `device_class`", "Dependabot keeps bumping actions", "this PR's version gate is red", "what's spamming my HA log".
...
- `.github/workflows/pr-commit-summary.yml` — **humans open their own PRs.** This maintains a generated, type-grouped commit list inside a marked block (`<!-- commit-summary:start -->…`) in the PR body, so release-drafter's `$BODY` still reads as a per-PR mini-changelog. It does **not** open PRs and does **not** touch the title. Body in `templates/.github/workflows/pr-commit-summary.yml`; rationale in `reference/github-actions.md`.
  - Triggers on `pull_request_target` (`opened`/`reopened`/`synchronize`) so it can write to PRs raised **from forks** — an external contributor's PR is the case it exists to serve. ⚠️ That trigger runs in the base repo's context with a writable token, so it **must never check out the PR head or run PR-authored code**. It doesn't: commit subjects come from the API into a file, and nothing from the PR is interpolated into a shell command.
  - Skips bot authors (`github.event.pull_request.user.type != 'Bot'`) — Dependabot already writes its own body and release-drafter's `replacers` scrub it.
  - Only the marked block is rewritten, so a human-written description survives every push. It's a no-op when the block is already current.
  - **Do NOT add a label step here** — labelling is the autolabeler's job (below); a second labeler makes labels flap.

  > **Superseded:** earlier versions of this skill shipped `create-dev-pr.yml`, which auto-opened a draft PR on every push to a non-main branch and derived the title from the commits. **Don't reinstate it.** It cannot serve fork-based contributions at all — a workflow on `push` never fires for a contributor pushing to their own fork, and a `pull_request` from a fork gets a read-only token, so it could not open the PR anyway. It also auto-opened a PR for every work-in-progress branch, overwrote human-edited PR titles on the next push, and tripped the `GITHUB_TOKEN` `opened`-suppression rule so first-open checks never ran. Manual PRs remove all four problems at once.
...
- `.github/workflows/pr-title-check.yml` — comments on a PR whose title the autolabeler can't map, suggesting a type derived from the PR's commits, and deletes its own comment once the title is fixed. **It never edits the title** — a workflow rewriting human titles is what got `create-dev-pr` removed. It decides by reading the PR's **actual labels**, not by re-implementing the autolabeler's regexes: a copy of that vocabulary here would drift from `.github/release-drafter.yml`, and a checker that disagrees with the thing it checks is worse than none. Triggering on `labeled`/`unlabeled` as well means it re-evaluates after the autolabeler acts, so it can't race `pr-labeler.yml`.
...
**Read `reference/github-actions.md` before changing any workflow** — it holds the must-preserve behaviours: the sole title-only labeler + removal-only superseded-label step, `$BODY` + bounded Dependabot `replacers`, the last-published-release version gate (with `dependabot[bot]` exempt and the unit-tested `manifest_gate.py`), the `pr-commit-summary` fork-safety and marked-block rules, and the optional personal reminder-hook recipe.
...
- **Workflows behave, not just exist:** `pr-commit-summary` runs on `pull_request_target`, **never checks out the PR head**, skips bot authors, rewrites only the marked block, and has no label step; no workflow opens PRs automatically; `release_drafter` is push-only with no second autolabeler; `check-manifest-version` compares to the **last published release** and exempts `dependabot[bot]` on the *failing steps*.
