...
- `.github/release-drafter.yml` — autolabeler rules are **title-only** (no `branch:` rules). The release-drafter autolabeler can only match title/body/branch/files (never commit subjects), so label off the **title**. Keep it the one-and-only labeler. ⚠️ Since a **human** now writes the title, the type vocabulary matters: the rules map only `feat|fix|chore|docs` (plus `!` → `xfeat`). A title typed `ci:`, `refactor:`, `build:`, `perf:`, `style:` or `revert:` matches nothing → no label → no release-drafter category → the version gate has no bump to check against. `lint_pr.yml` accepts those types, so it won't save you. Use a mapped type on the PR title (`chore:` for a workflow-only PR), whatever the individual commits say.
