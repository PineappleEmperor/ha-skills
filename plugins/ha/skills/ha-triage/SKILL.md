...
2. **Stale cached image** — the killer, and the mechanism is usually misdiagnosed. A snapshot written to a **fixed** `/config/www/…` filename and served via `/local/…` is sent with HA's own long-lived `Cache-Control` header on static paths (the value is in the table above), not a CDN's. The phone/OS/app HTTP cache then serves you the *previous* image (or a pre-first-write 404). It does **not** go away off Nabu Casa. Compounded by a write→push race, where the push beats the file flush.
